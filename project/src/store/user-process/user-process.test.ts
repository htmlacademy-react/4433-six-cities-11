...
import {InitialState as UserProcess} from './user-process';
...
describe('Reducer: user', () => {
  ...
  beforeEach(() => {
    state = {authorizationStatus: AuthorizationStatus.Unknown, userData: undefined};
  });
  ...
  describe('checkAuthAction test', () => {
    it('should update authorizationStatus to "AUTH" if checkAuthAction fulfilled', () => {
      expect(userProcess.reducer(state, { type: checkAuthAction.fulfilled.type }))
        .toEqual({authorizationStatus: AuthorizationStatus.Auth, authUser: undefined});
    });
